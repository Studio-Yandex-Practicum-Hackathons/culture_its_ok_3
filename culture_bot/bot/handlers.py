...
async def exhibit(message: Message, state: FSMContext):
    ...
    if (
        ...
        await message.answer(messages.END_WAY_MESSAGE)
        await message.answer(messages.WRITE_YOUR_OPINION)
...
async def after_get_review_message(message: Message, state: FSMContext):
    ...
    await message.answer(messages.RAITING_REVIEW)
...
async def after_get_review_message(message: Message, state: FSMContext):
    data = await state.get_data()
    await create_route_review_and_raiting(
        text=data['review_text'],
        telegram_id=message.from_user.id,
        route_id=way_counter[message.from_user.id][0],
        rating_route=message.text
    )
    await update_userfeedback(
        telegram_id=message.from_user.id,
        route_id=way_counter[message.from_user.id][0]
    )
    await message.answer(
